...
if (process.env.NAPCAT_URL) {
    NAPCAT_URL = process.env.NAPCAT_URL;
}
if (process.env.NAPCAT_TOKEN) {
    NAPCAT_TOKEN = process.env.NAPCAT_TOKEN;
}
if (process.env.NAPCAT_QQ) {
    NAPCAT_QQ = process.env.NAPCAT_QQ;
}
...
async function napcatNotify(text, desp) {
    return new Promise(resolve => {
        if (!(NAPCAT_URL && NAPCAT_TOKEN && NAPCAT_QQ)) {
            resolve();
            return;
        }
        send({
            method: 'POST',
            url: `${NAPCAT_URL.replace(/\/$/, '')}/send_private_msg`,
            headers: {
                'authorization': `Bearer ${NAPCAT_TOKEN}`,
                'content-type': 'application/json'
            },
            contents: {
                user_id: Number(NAPCAT_QQ),
                message: `${text}\n\n${desp}`
            },
            config: { retry: false, timeout: 15000 },
            success: res => {
                try {
                    const data = JSON.parse(res.body);
                    data.status === 'ok'
                        ? log.info('发送通知', 'NapCat QQ发送通知成功')
                        : log.error('发送通知', `NapCat QQ发送失败: ${res.body}`);
                } catch (e) {
                    log.error('发送通知', `NapCat QQ响应解析失败: ${e.message}`);
                } finally {
                    resolve();
                }
            },
            failure: err => {
                log.error('发送通知', `NapCat QQ调用失败: ${err}`);
                resolve();
            }
        });
    });
