...
const bili_client = {
    ...
    async getAttentionList(uid) {
        ...
        // B站已下线旧 feed/get_attention_list 接口(404)。该列表只用于
        // 关注状态提示，失败时降级为空列表，不能阻断抽奖主流程。
        const res = strToJson(responseText);
        if (res.code === 0 && res.data && Array.isArray(res.data.list)) {
            log.info('获取关注列表', '成功');
            return res.data.list.toString();
        }
        if (typeof responseText === 'string' && /HTTP状态码: 404|<!DOCTYPE HTML/i.test(responseText)) {
            log.warn('获取关注列表', '旧接口已下线，已降级为空列表');
        } else {
            log.warn('获取关注列表', '接口返回异常，已降级为空列表');
        }
        return '';
    },
    /**
    ...
    _getOneDynamicByDyid: new Line(
        ...
        , responseText => {
            const
                res = strToJson(responseText),
                { code, data } = res;
            // B站 412 request was banned 是服务端风控，继续重试只会扩大限流。
            if (/HTTP状态码: 412|request was banned/i.test(responseText)) {
                return [false, undefined, 'B站返回412风控，跳过当前动态'];
            }
            switch (code) {
                case 0:
